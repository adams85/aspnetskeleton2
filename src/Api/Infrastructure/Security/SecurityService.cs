...
namespace WebApp.Api.Infrastructure.Security
{
    public sealed class SecurityService : ISecurityService
    {
        ...
        public SecurityService(ICommandDispatcher commandDispatcher, IQueryDispatcher queryDispatcher, IGuidProvider guidProvider, IClock clock, IOptions<SecurityOptions>? options, ILogger<SecurityService>? logger)
        {
            ...
            {
                _logger.LogWarning($"{SecurityOptions.DefaultSectionName}:{nameof(SecurityOptions.JwtIssuerSigningKey)} was not configured. A temporary key is generated but be aware that the JWT tokens issued will not be accepted across multiple executions of the application.");
            ...
            var jwtAccessTokenClockSkew = optionsValue?.JwtAccessTokenClockSkew ?? SecurityOptions.DefaultJwtAccessTokenClockSkew;
            ...
            _jwtAccessTokenExpirationTime = optionsValue?.JwtAccessTokenExpirationTime ?? SecurityOptions.DefaultJwtAccessTokenExpirationTime;
            _jwtRefreshTokenExpirationTime = optionsValue?.JwtRefreshTokenExpirationTime ?? SecurityOptions.DefaultJwtRefreshTokenExpirationTime;
        ...
        private async Task ValidateJwtTokenAsync(TokenValidatedContext context)
        {
            ...
            claimsIdentity.AddClaim(new Claim(ClaimTypes.NameIdentifier, userInfo.UserId.ToString()));
            ...
            if (!string.IsNullOrEmpty(userInfo.FirstName))
                claimsIdentity.AddClaim(new Claim(ClaimTypes.GivenName, userInfo.FirstName));

            if (!string.IsNullOrEmpty(userInfo.LastName))
                claimsIdentity.AddClaim(new Claim(ClaimTypes.Surname, userInfo.LastName));

            if (userInfo.Roles != null)
                for (int i = 0, n = userInfo.Roles.Length; i < n; i++)
                    claimsIdentity.AddClaim(new Claim(ClaimTypes.Role, userInfo.Roles[i]));
