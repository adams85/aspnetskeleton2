...
namespace WebApp.UI.Models.DataTables
{
    public sealed class DataTableModel
    {
        ...
        private Func<IUrlHelper, object?, string> _generateDisplayUrl = (url, values) => url.Action(DefaultDisplayActionName, values);
        ...
        public Func<IUrlHelper, object?, string> GenerateCreateUrl { get; set; } = (url, values) => url.Action(DefaultCreateActionName, values);
        public Func<IUrlHelper, object?, string> GenerateEditUrl { get; set; } = (url, values) => url.Action(DefaultEditActionName, values);
        public Func<IUrlHelper, object?, string> GenerateDeleteUrl { get; set; } = (url, values) => url.Action(DefaultDeleteActionName, values);
